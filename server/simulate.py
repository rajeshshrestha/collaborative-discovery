...
# s: scenario #
# b_type: Bayesian type (i.e. what kind of user are we simulating) ("oracle", "informed", "uninformed")
# decision_type: coin-flip or probability threshold decision making ("coin-flip", "threshold")
...
def run(s, b_type, decision_type, stat_calc):
    ...
    for h in h_space:
        ...
        h['vio_pairs'] = set(tuple(vp) for vp in h[
            'vio_pairs'])  # Get violation pairs in the form of the sets of tuples

        '''
            Compute alpha and beta for user simulation using mu and variance,
                Get the confidence as the mu for the oracle from the clean dataset itself with variance=0.00000001
                For informed, use the conf computed from the dirty data with variance=0.01
    ...
    try:
        r = requests.post('http://localhost:5000/duo/api/import', data={
            ...
            # TODO: Add command-line argument for close or far violation ratio (e.g. 3:1 or 3:2). This may change.
        })
        print(r)
        res = r.json()
    ...
    try:
        ...
        sample_X = set(tuple(x) for x in res[
            'X'])  ### list of true violations(with correct assumed based on the clean data) with respect to target FD in dirty data
        print('got vios')
        data = json.loads(sample)
        feedback = json.loads(res['feedback'])
        print('parsed data')

        # ! Change the sample to string type with None replaced by '' for all the fields in the sample except for the id field
        for row in data.keys():
    ...
    while msg != '[DONE]' and (len(data) > 0):
        ...
        feedbackMap = buildFeedbackMap(data, feedback,
                                       header)  # Initialize feedback dictionary utilized during interaction
        ...
        for fd, fd_m in fd_metadata.items():
            ...
            # ? Not updating incase of oracle. Does it mean oracle knows all and no need for hyperparameter update.
            if b_type != 'oracle':
                successes = 0
                failures = 0
        ...
        # fd_m = fd_metadata[
        #     target_fd]  # Pick an FD to make decisions off of (i.e. hypothesize an FD). TODO: Don't just use target_fd

        selected_fd = random.choices([old_max_fd, max_fd], weights=[0.3, 0.7])[0]
        fd_m = fd_metadata[selected_fd]
        q_t = fd_m.alpha / (
                    fd_m.alpha + fd_m.beta) if b_type != 'oracle' else fd_m.conf  # ! account for update of alpha and beta incase of b_type not being oracle
        ...
        for row in data.keys():
            ...
            if b_type is oracle
                ...
                else stat_calc=recall and len(iter_vios_marked) >= max_marked, q_t=0
            '''
            ...
            vios_w_i = {v for v in iter_vios_total if
                        int(row) in v and v not in iter_vios_marked}  # Find all violations that involve this row
            ...
            if decision == 1:  # User correctly figured out the truth for this tuple with respect to this FD
                if len(vios_w_i) > 0:  # This tuple is involved in a violation of our hypothesized FD
                    '''
                    If decision 1 obtained and actual not marked violations present including that row is greater than 0,
...
if __name__ == '__main__':
    ...
    b_type = sys.argv[2]  # Bayesian type ("oracle", "informed", "uninformed", "random")
